...
public class Animation
{
    ...
    static private final Logger logger = LoggerFactory.getLogger(Animation.class);
    static final String VERSION = "2.6";

    public static final Map<String, TextColor> COLOR_MAP;


    static
    {
        //  create lists mapping full color names (eg. 'blue') and single character color ids (eg. 'b'). They better be unique!
        Map<String, TextColor> temp_color_names = new HashMap<>(16);
        temp_color_names.putAll(
                Map.ofEntries(entry("BLACK",  TextColor.ANSI.BLACK),entry("K",  TextColor.ANSI.BLACK),
                        entry("WHITE", TextColor.ANSI.WHITE),entry("W", TextColor.ANSI.WHITE),
                        entry("RED", TextColor.ANSI.RED),    entry("R", TextColor.ANSI.RED),
                        entry("GREEN", TextColor.ANSI.GREEN),entry("G", TextColor.ANSI.GREEN),
                        entry("BLUE", TextColor.ANSI.BLUE),  entry("B",TextColor.ANSI.BLUE),
                        entry("CYAN", TextColor.ANSI.CYAN),  entry("C", TextColor.ANSI.CYAN),
                        entry("MAGENTA", TextColor.ANSI.MAGENTA),entry("M", TextColor.ANSI.MAGENTA),
                        entry("YELLOW", TextColor.ANSI.YELLOW),entry("Y",TextColor.ANSI.YELLOW)));

        COLOR_MAP = temp_color_names;

    }

    private TextColor background = TextColor.ANSI.BLACK;
    private List<Entity> entities = new ArrayList<>();
    private List<Entity> physicalEntities = new ArrayList<>();
//    private List<Entity> collissions
    private boolean color_enabled = false;
    private long last_frame_time = 0;

    //  framerate related settings
    private boolean track_framerate = true;
    ...
    {
        this(new DefaultTerminalFactory().createTerminal());
        fullscreen = false;
    }
     ...
     * @throws IOException
     */
    public Animation(Terminal terminal) throws IOException
    {
        win = terminal;
...
// // // // // // // // // // // // // // // // // // // // //  COLOR UTILITIES // // // // // // // // // // // // // // // // // // // // // // // 
    ...
    /**
     * Returns the full name of a color, given a single character abbreviation.
     *
     * @param color
     * @return
     */
    private TextColor color_name(String color)
    {
        TextColor name = COLOR_MAP.get(color);
        ...
        {
            logger.error("Attempt to allocate unknown color: {}", color);
            return null;
        }
    }
     ...
     *
     * @todo this is all lies, do we need this?
     *
     * @param color
     * @return
     */
    private String color_id(String color)
    {
        TextColor id = COLOR_MAP.get(color);
     ...
     * @return
     */
    private boolean is_valid_color(String color)
    {
...
//////////////////////////////////////////////////// PHYSICS UTILITIES ////////////////////////////////////////////////////
        ...
        // collisions.
    private void find_collisions() {

//    my @col_set = ();
    List<Point3D> coord = new ArrayList<>(entities.size());
        List<Point3D> size = new ArrayList<>(entities.size());
    List<String> name = new ArrayList<>(entities.size());

    for (Entity ent: entities){
        if(ent.getPhysical()){
            coord.add(ent.getPosition());
            size.add(ent.getSize());
            name.add(ent.getName());

        for (int i =0;i<coord.size();i++) {
			// X
            if( (coord.get(i).getX() <= coord.get(coord.size()-1).getX() && coord.get(coord.size()-1).getX() < coord.get(i).getX() + size.get(i).getX()) ||
                    (coord.get(coord.size()-1).getX() <= coord.get(i).getX() && coord.get(i).getX() < coord.get(-1).getX() + size.get(size.size()-1).getX()) ) {
				// Y
                if( (coord.get(i).getY() <= coord.get(coord.size()-1).getY() && coord.get(coord.size()-1).getY() < coord.get(i).getY() + size.get(i).getY()) ||
                        (coord.get(coord.size()-1).getY() <= coord.get(i).getY() && coord.get(i).getY() < coord.get(coord.size()-1).getY() + size.get(size.size()-1).getY()) ){
                    // Z
                    if ((coord.get(i).getZ() <= coord.get(coord.size()-1).getZ() && coord.get(coord.size()-1).getZ() <coord.get(i).getZ() + size.get(i).getZ()) ||
                            (coord.get(coord.size()-1).getZ() <= coord.get(i).getZ() && coord.get(i).getZ() < coord.get(coord.size()-1).getZ() + size.get(size.size()-1).getZ()) ){
...
//	# clear the window before we start redrawing
        ...
        {
            logger.error("Unable to refresh screen '{}",e);
        }
    }
...
//    my $attr;
    ...
    for (int i=0;i<shape.length;i++){
        ...
        for (int j=0;j<shape[i].length;j++) {
            if(shape[i][j] != null && shape[i][j].getCharacter() != entity.getTransparent()) { // transparent char
                int x_pos = x+j;

                if(entity.isWrap()) {
                    while(x_pos >= w) { x_pos -= w; }
                    while(y_pos >= h) { y_pos -= h; }
                } else if(x_pos >= w || y_pos >= h) {
                    continue;
                }

                if(x_pos >= 0 && y_pos >= 0 ) {
                    if(true /*$color_enabled*/) {
                        screen.setCharacter(x_pos,y_pos,shape[i][j]);
